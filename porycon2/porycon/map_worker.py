...
def convert_single_map(args_tuple):
    ...
    map_id, map_info, input_dir, output_dir, layouts_dict, region_override, warp_lookup, tiled_mode = args_tuple

    try:
        from .converter import MapConverter
        from .utils import load_json

        # Create a converter instance for this worker (pass tiled_mode to skip Definition files)
        local_converter = MapConverter(str(input_dir), str(output_dir), tiled_mode=tiled_mode)
