...
class WorldBuilder:
    ...
    def save_world(self, world_name: str, tiled_mode: bool = False):
        ...
        if tiled_mode:
            # For Tiled mode, save to root Worlds/ directory
            world_path = self.output_dir / "Worlds" / f"{world_name}.world"
        else:
            # For entity mode, save to Definitions/Worlds/
            world_path = self.output_dir / "Definitions" / "Worlds" / f"{world_name}.world"
        world_path.parent.mkdir(parents=True, exist_ok=True)
        save_json(world, str(world_path))

    def save_all_worlds(self, tiled_mode: bool = False):
        """Save all world files."""
        for world_name in self.worlds:
            self.save_world(world_name, tiled_mode=tiled_mode)
