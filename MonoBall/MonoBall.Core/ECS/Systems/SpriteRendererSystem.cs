                ...
                if (needsShaderStacking && renderTarget != null)
                {
                    ...
                    try
                    {
                        ...
                        foreach (var (entity, spriteId, anim, pos, render) in sprites)
                        {
                            ...
                            var activeShader = entityShader; // No layer shader - ApplyShaderStack will handle it
            ...
            if (!needsShaderStacking)
            {
                ...
                sprites.Sort(
                    (a, b) =>
                    {
                        var shaderA = GetEntityShader(a.entity);
                        var shaderB = GetEntityShader(b.entity);
                        var activeA = shaderA ?? spriteLayerShader;
                        var activeB = shaderB ?? spriteLayerShader;
                ...
                foreach (var (entity, spriteId, anim, pos, render) in sprites)
                {
                    ...
                    var entityShader = GetEntityShader(entity);
                    var activeShader = entityShader ?? spriteLayerShader;
