...
public class MapBorderRendererSystem : BaseSystem<World, float>
{
    ...
    private readonly List<MapBorderInfo> _cachedMapBorders = new();
    private readonly List<MapBoundsInfo> _cachedMapBounds = new();
    ...
    {
        RenderBorderLayer(gameTime, false);
    }
    ...
    {
        RenderBorderLayer(gameTime, true);
    }
    ...
    private void RenderBorderLayer(GameTime gameTime, bool useTopLayer)
    {
        ...
        try
        {
            ...
            for (var tileX = expandedTileBounds.X; tileX < expandedTileBounds.Right; tileX++)
            {
                ...
                // Draw the tile
                _spriteBatch.Draw(tilesetTexture, tilePixelPosition, sourceRect, Color.White);
            }
    ...
    private void UpdateCachedData(CameraComponent camera)
    {
        ...
        var currentMapCount = World.CountEntities(in _mapBorderQuery);
        if (
        ...
        )
        {
            ...
            var currentTileViewBounds = camera.GetTileViewBounds();
        ...
        World.Query(
            ...
            ) =>
            {
                ...
                {
                    _logger.Warning("Map {MapId} has invalid border component arrays", map.MapId);
                    return;
                }

                // Calculate map bounds in tile coordinates
                var mapOriginTileX = (int)(mapPosition.Position.X / map.TileWidth);
                var mapOriginTileY = (int)(mapPosition.Position.Y / map.TileHeight);
                var mapRightTile = mapOriginTileX + map.Width;
                var mapBottomTile = mapOriginTileY + map.Height;
        ...
        // Cache camera bounds
        var newTileViewBounds = camera.GetTileViewBounds();
        _cachedCameraBounds = new Rectangle(
