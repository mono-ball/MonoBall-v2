...
namespace MonoBall.Core.ECS.Systems
{
    ...
    public class CameraSystem : BaseSystem<World, float>
    {
        ...
        /// <exception cref="InvalidOperationException">Thrown if entity lacks required components or frame rectangle cannot be retrieved.</exception>
        ...
        private static Vector2 ConvertPixelToTile(
        ...
        )
        {
            if (tileWidth <= 0)
            {
                throw new ArgumentException(
                    "Tile width must be greater than zero.",
                    nameof(tileWidth)
                );
            }
        ...
        private void UpdateCamera(ref CameraComponent camera, float deltaTime)
        {
            ...
            {
                Entity followEntity = camera.FollowEntity.Value;
                ...
                else
                {
                    ...
                    Vector2 tilePos = ConvertPixelToTile(
                        entityCenter,
                        camera.TileWidth,
                        camera.TileHeight
                    );
        ...
        public void SetCameraFollowEntity(Entity cameraEntity, Entity targetEntity)
        {
            ...
            // Calculate the center point of the entity's sprite for proper camera centering
            Vector2 entityCenter = CalculateEntityCenter(targetEntity, targetPos.Position);

            // Convert pixel position to tile coordinates using camera's tile dimensions
            Vector2 tilePos = ConvertPixelToTile(entityCenter, camera.TileWidth, camera.TileHeight);
