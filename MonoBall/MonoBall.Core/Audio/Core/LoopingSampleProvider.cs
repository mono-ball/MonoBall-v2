        ...
        public int Read(float[] buffer, int offset, int count)
        {
            lock (_lock)
            {
                ...
                while (totalSamplesRead < count)
                {
                    ...
                    long loopEnd = _loopEndSamples ?? _source.TotalSamples;
                    long loopStart = _loopStartSamples ?? 0;
                    ...
                    long samplesUntilLoopEnd = loopEnd - currentPosition;
                    int samplesToRead = (int)
                        Math.Min(samplesUntilLoopEnd, count - totalSamplesRead);

                    // Read samples from source
                    int samplesRead = _source.Read(
                        buffer,
