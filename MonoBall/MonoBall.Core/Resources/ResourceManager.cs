...
public class ResourceManager : IResourceManager, IDisposable
{
    ...
    public Texture2D LoadTexture(string resourceId)
    {
        ...
        var relativePath = ExtractTexturePath(resourceId);
        var virtualPath = _pathResolver.ResolveResourcePath(resourceId, relativePath);
        ...
        var fileData = modManifest.ModSource.ReadFile(actualRelativePath);
    ...
    public FontSystem LoadFont(string resourceId)
    {
        ...
        if (testFont == null)
            throw new InvalidOperationException($"Font file is invalid or corrupted: {resourceId}");
    ...
    public VorbisReader LoadAudioReader(string resourceId)
    {
        ...
        var audioData = modManifest.ModSource.ReadFile(actualRelativePath);
    ...
    public Effect LoadShader(string resourceId)
    {
        ...
        var virtualPath = _pathResolver.ResolveResourcePath(resourceId, shaderDef.SourceFile);
        ...
        var bytecode = modManifest.ModSource.ReadFile(actualRelativePath);
    ...
    )
    {
        ...
        {
            // Resolve variable sprites - fail fast if cannot resolve
            var actualSpriteId = ResolveVariableSpriteIfNeeded(spriteId, "animation frames");
    ...
    public TilesetDefinition GetTilesetDefinition(string tilesetId)
    {
        ...
        if (string.IsNullOrEmpty(tilesetId))
            throw new ArgumentException("Tileset ID cannot be null or empty.", nameof(tilesetId));
        ...
        lock (_lock)
        {
            ...
            if (definition == null)
                throw new InvalidOperationException($"Tileset definition not found: {tilesetId}");
    ...
    public Rectangle CalculateTilesetSourceRectangle(string tilesetId, int gid, int firstGid)
    {
        ...
        var column = localTileId % definition.Columns;
        var row = localTileId / definition.Columns;

        // Calculate source rectangle accounting for spacing and margin
        var sourceX = column * (definition.TileWidth + definition.Spacing) + definition.Margin;
        var sourceY = row * (definition.TileHeight + definition.Spacing) + definition.Margin;
    ...
    public string LoadTextFile(string resourceId, string relativePath)
    {
        ...
        var textContent = modManifest.ModSource.ReadTextFile(actualRelativePath);

        _logger.Debug("Loaded text file: {ResourceId} -> {RelativePath}", resourceId, relativePath);
    ...
    private void EvictLRUTexture()
    {
        ...
        {
            var lruKey = _textureAccessOrder.Last?.Value;
    ...
    private void EvictLRUShader()
    {
        ...
        {
            var lruKey = _shaderAccessOrder.Last?.Value;
