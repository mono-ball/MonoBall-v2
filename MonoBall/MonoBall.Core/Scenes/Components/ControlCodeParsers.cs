...
{
    private static readonly Dictionary<string, IControlCodeParser> _parsers = new(
    ...
    public static bool TryParse(string controlCode, int originalPosition, out TextToken token)
    {
        ...
        if (_parsers.TryGetValue(controlCode, out var exactParser) && !exactParser.IsParameterized)
            try
            {
        ...
        foreach (var kvp in _parsers)
        {
            ...
            {
                var prefix = parser.ControlCodeName + ":";
                if (
...
internal class PauseParser : IControlCodeParser
{
    ...
    public TextToken Parse(string controlCode, int originalPosition)
    {
        ...
        var secondsStr = controlCode.Substring(6); // Skip "PAUSE:"
        if (
            !float.TryParse(
                secondsStr,
                NumberStyles.Float,
...
internal class ColorParser : IControlCodeParser
{
    ...
    public TextToken Parse(string controlCode, int originalPosition)
    {
        ...
        var colorStr = controlCode.Substring(6); // Skip "COLOR:"
        ...
        var colorValue = new Color(r, g, b, (byte)255);
        return new TextToken
        {
...
internal class ShadowParser : IControlCodeParser
{
    ...
    public TextToken Parse(string controlCode, int originalPosition)
    {
        ...
        var shadowStr = controlCode.Substring(7); // Skip "SHADOW:"
        var parts = shadowStr.Split(',');
        if (
            parts.Length != 3
            || !byte.TryParse(parts[0].Trim(), out var r)
            || !byte.TryParse(parts[1].Trim(), out var g)
        ...
        var shadowValue = new Color(r, g, b, (byte)255);
        return new TextToken
        {
...
internal class SpeedParser : IControlCodeParser
{
    ...
    public TextToken Parse(string controlCode, int originalPosition)
    {
        ...
        var speedStr = controlCode.Substring(6); // Skip "SPEED:"
...
internal class EffectStartParser : IControlCodeParser
{
    ...
    public TextToken Parse(string controlCode, int originalPosition)
    {
        ...
        var effectId = controlCode.Substring(3); // Skip "FX:"
