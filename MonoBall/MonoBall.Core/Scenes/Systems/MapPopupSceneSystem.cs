        ...
        ISceneSystem
{
    ...
    [Obsolete("MapPopupSystem now handles popup destruction. This method should not be called.")]
    ...
    )
    {
        ...
        var viewportHeight = camera.Viewport.Height;
    ...
    {
        if (outlineDef.TileUsage == null || outlineDef.Tiles == null || outlineDef.Tiles.Count == 0)
            return;
        ...
        var tileW = outlineDef.TileWidth * scale;
        var tileH = outlineDef.TileHeight * scale;
        ...
        for (var i = 0; i < usage.TopEdge.Count && i < 12; i++)
        {
            ...
            var tileX = interiorX + (i - 1) * tileW; // i-1 means first tile is at x-1 (pokeemerald: i - 1 + x)
            var tileY = interiorY - tileH; // y-1 in tile units (pokeemerald: y - 1)
        ...
        {
            var popupInteriorTilesYLeftEdge = _constants.Get<int>("PopupInteriorTilesY");
            for (var i = 0; i < usage.LeftEdge.Count && i < popupInteriorTilesYLeftEdge; i++)
            {
        ...
        else if (usage.LeftMiddle != 0)
        {
            ...
            for (var i = 0; i < popupInteriorTilesYLeft; i++)
        ...
        if (usage.RightEdge != null && usage.RightEdge.Count > 0)
        {
            ...
            var popupInteriorTilesX = _constants.Get<int>("PopupInteriorTilesX");
            for (var i = 0; i < usage.RightEdge.Count && i < popupInteriorTilesYRight; i++)
            {
                var tileIndex = usage.RightEdge[i];
                DrawTile(tileIndex, interiorX + popupInteriorTilesX * tileW, interiorY + i * tileH);
            }
        }
        ...
        for (var i = 0; i < usage.BottomEdge.Count && i < 12; i++)
        {
            ...
            var tileX = interiorX + (i - 1) * tileW;
            var tileY = interiorY + _constants.Get<int>("PopupInteriorTilesY") * tileH;
            DrawTile(tileIndex, tileX, tileY);
        }
    }
    ...
    )
    {
        ...
        var srcCornerW = outlineDef.CornerWidth ?? 8;
        var srcCornerH = outlineDef.CornerHeight ?? 8;
        var texWidth = outlineTexture.Width;
        var texHeight = outlineTexture.Height;

        // Scaled destination dimensions
        var destCornerW = srcCornerW * scale;
        var destCornerH = srcCornerH * scale;
    ...
    private Texture2D? LoadBackgroundTexture(string definitionId)
    {
        ...
        {
            _logger.Warning(ex, "Failed to load background texture: {DefinitionId}", definitionId);
            return null;
        }
    }
