...
public class WindowAnimationSystem : BaseSystem<World, float>, IPrioritizedSystem
{
    ...
    )
    {
        ...
        var currentPhaseIndex = GetCurrentPhaseIndex(ref anim);
        ...
        var phaseStartTime = GetCumulativePhaseTime(ref anim, currentPhaseIndex);
        var phaseElapsedTime = anim.ElapsedTime - phaseStartTime;
        ...
        else
        {
            ...
            var progress = Math.Min(phaseElapsedTime / phase.Duration, 1.0f);
            var easedProgress = ApplyEasing(progress, phase.Easing);
    ...
    private int GetCurrentPhaseIndex(ref WindowAnimationComponent anim)
    {
        ...
        for (var i = 0; i < anim.Config.Phases.Count; i++)
        {
    ...
    private float GetCumulativePhaseTime(ref WindowAnimationComponent anim, int phaseIndex)
    {
        ...
        var time = 0f;
        for (var i = 0; i < phaseIndex && i < anim.Config.Phases.Count; i++)
