...
public abstract class ScriptBase : IDisposable
{
    ...
    ///     Thrown if this is a plugin script or entity doesn't have GridMovement
    ///     component.
    /// </exception>
    ...
    protected void StartTimer(string timerId, float duration, bool isRepeating = false)
    {
        ...
        if (Context.Entity == null)
            throw new InvalidOperationException("Cannot start timer on plugin script (no entity)");

        if (string.IsNullOrWhiteSpace(timerId))
            throw new ArgumentException("Timer ID cannot be null or empty", nameof(timerId));

        if (duration <= 0)
            throw new ArgumentException("Timer duration must be greater than 0", nameof(duration));
    ...
    protected void UpdateTimer(string timerId, float newDuration)
    {
        ...
        if (Context.Entity == null)
            throw new InvalidOperationException("Cannot update timer on plugin script (no entity)");
    ...
    protected void CancelTimer(string timerId)
    {
        ...
        if (Context.Entity == null)
            throw new InvalidOperationException("Cannot cancel timer on plugin script (no entity)");
    ...
    {
        var str = Context.GetParameter<string>(name);
    ...
    {
        return Context.GetParameter(name, defaultValue);
    }
    ...
    {
        return (Get(keyX, defaultX), Get(keyY, defaultY));
    }
