...
public static class CameraTransformUtility
{
    ...
    public static Matrix GetTransformMatrix(CameraComponent camera)
    {
        ...
        var pixelX = camera.Position.X * camera.TileWidth;
        var pixelY = camera.Position.Y * camera.TileHeight;
        ...
        var roundedX = MathF.Round(pixelX * camera.Zoom) / camera.Zoom;
        var roundedY = MathF.Round(pixelY * camera.Zoom) / camera.Zoom;

        // Use Viewport width/height for centering (VirtualViewport is same size, just offset)
        var centerX = camera.Viewport.Width / 2f;
        var centerY = camera.Viewport.Height / 2f;
    ...
    {
        var matrix = GetTransformMatrix(camera);
        Matrix.Invert(ref matrix, out var invertedMatrix);
        var pixelPos = Vector2.Transform(screenPosition, invertedMatrix);
    ...
    public static RectangleF GetBoundingRectangle(CameraComponent camera)
    {
        ...
        var worldViewportWidth = camera.Viewport.Width / camera.Zoom;
        var worldViewportHeight = camera.Viewport.Height / camera.Zoom;

        // Convert to tile coordinates
        var viewportWidthTiles = worldViewportWidth / camera.TileWidth;
        var viewportHeightTiles = worldViewportHeight / camera.TileHeight;

        var halfWidth = viewportWidthTiles / 2f;
        var halfHeight = viewportHeightTiles / 2f;
    ...
    {
        var bounds = GetBoundingRectangle(camera);
    ...
    public static Vector2 ClampPositionToMapBounds(CameraComponent camera, Vector2 position)
    {
        ...
        var halfViewportWidth = viewportWidthTiles / 2f;
        var halfViewportHeight = viewportHeightTiles / 2f;
