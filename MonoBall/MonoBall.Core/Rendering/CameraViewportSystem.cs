...
/// </summary>
public class CameraViewportSystem : BaseSystem<World, float>, IPrioritizedSystem
{
    ...
    )
    {
        ...
        camera.VirtualViewport = new Rectangle(virtualX, virtualY, viewportWidth, viewportHeight);
        ...
        else
        {
            ...
            var previousScaleX = Math.Max(1, camera.ReferenceWidth / referenceWidth);
            var previousScaleY = Math.Max(1, camera.ReferenceHeight / referenceHeight);
            var previousScale = Math.Min(previousScaleX, previousScaleY);

            // Scale zoom proportionally to maintain same world view
            if (previousScale > 0)
            {
                var zoomRatio = (float)scale / previousScale;
