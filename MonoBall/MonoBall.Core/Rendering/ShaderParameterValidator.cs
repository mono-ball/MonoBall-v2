...
public class ShaderParameterValidator : IShaderParameterValidator, IDisposable
{
    ...
    )
    {
        ...
        var isValid = parameter.ParameterType switch
        {
        ...
        if (_modManager != null)
        {
            ...
            {
                var paramDef = shaderDef.Parameters.FirstOrDefault(p => p.Name == parameterName);
                if (paramDef != null)
                {
                    ...
                    {
                        bool typeMatches;
                        var typeLower = paramDef.Type.ToLowerInvariant();
                        switch (typeLower)
                        {
                            case "float":
