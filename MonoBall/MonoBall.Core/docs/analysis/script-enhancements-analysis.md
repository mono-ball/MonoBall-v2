...
Analysis of the script enhancements including `ScriptBase` helpers, `INpcApi`, `DirectionHelper`, and `RandomHelper` utilities. Identified several architectural issues, DRY violations, and potential bugs.
...
- `ScriptBase.GetFacingDirection()` / `TryGetFacingDirection()` and `INpcApi.GetFacingDirection()` both retrieve facing direction
...
**Options:**
1. **Remove ScriptBase helpers**, migrate scripts to use `Context.Apis.Npc.FaceDirection(Context.Entity.Value, direction)`
...
**Recommended:** Option 3 - Keep convenience API but eliminate duplication by having ScriptBase methods delegate to INpcApi.
...
> **NO FALLBACK CODE** - Fail fast with clear exceptions, never silently degrade or use default values for required dependencies
...
**Issue:**
`IsEventForThisEntity<TEvent>(ref TEvent evt)` copies struct then boxes for reflection. Need to verify this works correctly for all event types.
...
**Issue:**
`entityProp.GetValue(boxedEvt)!` uses null-forgiving operator, but if property exists but returns null, this could throw.
...
1. **HIGH PRIORITY:**
   ...
   - [ ] Consider removing duplicate functionality between ScriptBase and INpcApi (or make ScriptBase delegate to INpcApi)
