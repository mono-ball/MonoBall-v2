...
Analysis of the implementation plan against the design document to identify issues, missing details, and inconsistencies.
...
- `MapPopupRendererSystem` should calculate base position (without animation) and pass `WindowAnimationComponent` to `WindowRenderer`
...
**Fix**: Update plan to clarify that `MapPopupRendererSystem` calculates base position, and `WindowRenderer` applies animation transformations.
...
- Current: `RenderPopup(Entity entity, ref MapPopupComponent popup, ref PopupAnimationComponent anim, CameraComponent camera)`
- Should be: `RenderPopup(Entity entity, ref MapPopupComponent popup, ref WindowAnimationComponent anim, CameraComponent camera)`
...
**Fix**: Add note: "Remove `Update()` override entirely (or keep empty override if BaseSystem requires it - check BaseSystem implementation)".
...
**Fix**: Add note: "Note: Opacity transformations are calculated but not applied (renderer interfaces don't support opacity yet - future enhancement)".
...
**Fix**: Add detail: "Remove `anim.CurrentY` usage - calculate base position without animation offset, let WindowRenderer apply PositionOffset".
...
**Fix**: Add note: "Validate `WindowAnimationComponent.WindowEntity` is alive before using animation component (optional safety check)".
...
- Design shows events are structs, so likely need `EventBus.Subscribe<WindowAnimationCompletedEvent>(OnWindowAnimationCompleted)` with ref action

**Fix**: Add example: "Subscribe using `EventBus.Subscribe<WindowAnimationCompletedEvent>(OnWindowAnimationCompleted)` pattern (check EventBus API for ref action support)".
...
**Fix**: Add explicit parameter signature: "Add optional nullable parameter: `WindowAnimationComponent? animation = null`".
...
**Fix**: Add note: "Implement internal `AnimationEvent` struct with `Fire()` method for deferred event firing (internal helper, not public API)".
...
1. **Update Phase 6.2** to clarify that `MapPopupRendererSystem` calculates base position and `WindowRenderer` applies animation transformations.
