...
This document analyzes the UI Window System implementation for architecture issues, SOLID/DRY violations, Arch ECS/event
issues, `.cursorrule` compliance, and definition file inconsistencies.
...
- `MessageBoxContentRenderer` implements `IContentRenderer` but requires `SetComponent()` to be called before
  `RenderContent()`.
- This violates the **Liskov Substitution Principle** - code using `IContentRenderer` cannot use
  `MessageBoxContentRenderer` without knowing about `SetComponent()`.
...
- **Option B**: Create specialized interface `IMessageBoxContentRenderer` with
  `RenderContent(ref MessageBoxComponent, ...)`.
...
❌ **VIOLATION**: `MessageBoxContentRenderer` cannot be substituted for `IContentRenderer` without calling
`SetComponent()` first.
