...
This document analyzes the implementation of the variable sprite resolution system for architecture issues, Arch ECS
issues, timing issues, SOLID/DRY violations, and bugs.
...
**Issue**: After resolving the variable sprite ID to `actualSpriteId`, the code still uses `npcDef.SpriteId` (the
original variable sprite ID) in two places:
...
- Animation validation will fail for variable sprites (tries to validate `{base:sprite:npcs/generic/var_rival}` instead
  of `base:sprite:players/brendan/normal`)
...
**Issue**: The resolution cache (`_resolutionCache`) is never cleared when game state variables change. If a variable
sprite's underlying game state variable changes after initial resolution, the cached value will be stale.
...
- If `base:sprite:npcs/generic/var_rival` changes from `base:sprite:players/brendan/normal` to
  `base:sprite:players/may/normal` during gameplay, NPCs created after the change will still use the cached old value
...
**Issue**: The design document mentions `OnVariableChanged()` method, but it's not implemented. There's no way to
automatically invalidate the cache when variables change.
...
**Severity**: **MEDIUM** - Cache invalidation is important but may not be needed if variables are immutable after
initialization
...
**Issue**: `SpriteLoaderService` attempts to resolve variable sprites as a safety net, but if `_variableSpriteResolver`
is null, it silently fails and returns null.
...
**Status**: ✅ **NOT AN ISSUE** - `VariableSpriteResolver` is a service, not a system, so it doesn't need
...
**Issue**: Catches `InvalidOperationException` and returns `null`, which could mask errors if the caller expects an
exception.
...
1. **Bug 1**: ✅ **FIXED** - Using `npcDef.SpriteId` instead of `actualSpriteId` in animation validation and sprite
   definition lookup (lines 862, 874)
...
1. **DRY Violation**: ✅ **FIXED** - Extracted resolution logic into `ResolveVariableSpriteIfNeeded()` helper method in
   `SpriteLoaderService`
2. **Code Quality**: ✅ **FIXED** - Standardized error messages across all locations with consistent format and context
3. **Timing**: ✅ **FIXED** - Added comprehensive documentation about runtime variable changes and cache invalidation
   behavior
...
1. **DRY Violation**: ✅ **FIXED** - Extracted `ResolveVariableSpriteIfNeeded()` helper method in `SpriteLoaderService`
   to eliminate code duplication between `GetSpriteDefinition()` and `GetSpriteTexture()`
...
The implementation is **fully solid** with excellent architecture, SOLID principles, and proper resource management. All
critical, high-priority, and low-priority issues have been addressed.
