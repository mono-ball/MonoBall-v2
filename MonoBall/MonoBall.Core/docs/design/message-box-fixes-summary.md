...
4. ❌ **Circular event dependency risk** - Firing `MessageBoxHideEvent` when destroying existing message box creates recursive call path
...
9. ❌ **Missing validation** - `ProcessCharacter()` accesses `ParsedText[CurrentTokenIndex]` without bounds check after initial check
