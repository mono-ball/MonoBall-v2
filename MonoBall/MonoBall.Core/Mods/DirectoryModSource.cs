...
namespace MonoBall.Core.Mods
{
    ...
    public class DirectoryModSource : IModSource, IDisposable
    {
        ...
        public DirectoryModSource(string directoryPath)
        {
            ...
            if (!Directory.Exists(directoryPath))
            {
                throw new DirectoryNotFoundException(
                    $"Mod directory does not exist: {directoryPath}"
                );
            }
        ...
        /// </summary>
        public string SourcePath => _directoryPath;
        ...
        public bool FileExists(string relativePath)
        {
            ...
            try
            {
                ...
                var fullPath = Path.Combine(_directoryPath, normalizedPath);
        ...
        public IEnumerable<string> EnumerateFiles(string searchPattern, SearchOption searchOption)
        {
            ...
            var files = Directory.GetFiles(_directoryPath, searchPattern, searchOption);
            ...
            lock (_manifestLock)
            {
                ...
                if (!FileExists("mod.json"))
                {
                    throw new InvalidOperationException(
                        $"mod.json not found in mod directory: {_directoryPath}"
                    );
                }

                var jsonContent = ReadTextFile("mod.json");
                _cachedManifest = ModManifestLoader.LoadFromJson(jsonContent, this, _directoryPath);
