...
namespace MonoBall.ArchiveTool
{
    ...
    public class ArchiveCreator
    {
        ...
        public void CreateArchive(string modDirectory, string outputPath, int compressionLevel = DefaultCompressionLevel)
        {
            ...
            long totalBytes = GetTotalSize(files);

            try
            {
                using (var writer = new BinaryWriter(File.Create(outputPath), Encoding.UTF8, leaveOpen: false))
                {
                    ...
                    for (int i = 0; i < files.Length; i++)
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                int maxCompressedSize = LZ4Codec.MaximumOutputSize(uncompressed.Length);
