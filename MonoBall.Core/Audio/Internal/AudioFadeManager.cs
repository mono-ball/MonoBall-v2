...
internal class AudioFadeManager
{
    ...
    public static void UpdateFadeIn(MusicPlaybackState state, float deltaTime)
    {
        ...
        var progress = Math.Min(state.FadeInTimer / state.FadeInDuration, 1.0f);
    ...
    public static bool UpdateFadeOut(MusicPlaybackState state, float deltaTime)
    {
        ...
        var progress = Math.Min(state.FadeOutTimer / state.FadeOutDuration, 1.0f);
        var startVol = state.StartVolume > 0 ? state.StartVolume : state.TargetVolume;
